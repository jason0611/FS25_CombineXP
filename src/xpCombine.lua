...
-- @TEST:
-- [ ] Start threshing without cutter
-- [ ] Start threshing with cutter attached
-- [ ] Start threshing with cutter attached & activated
-- [ ] Start threshing with combine folded
-- [ ] Start threshing with cutter folded
-- [ ] Start Worker without cutter
-- [ ] Start Worker with cutter attached
-- [ ] Start Worker with cutter attached & activated
-- [ ] Start Worker with combine folded
-- [ ] Start Worker with cutter folded
-- [ ] Try harvesting with cutter deactivated
-- [ ] Harvesting at 5am
...
-- [ ] Open settings in quickhelp disappear after 1st use
-- [ ] In hosted MP, setting not working on client
-- [ ] Update mod preview

-- [ ] Move xpCombine.mrCombineLimiter.powerBoost to g_combinexp + impacts
-- [ ] Check if Event is needed for MP/Dedi Settings
-- [ ] Set all settings in Event if needed
-- [ ] Move settings to Game settings instead of General (initGameSettingsGui)
-- [ ] Manage load/save settings
...
function xpCombine:onLoad(savegame)
    ...
    -- Then motorConfiguration hp
        ...
        local coef = 1.5 -- It was 1.5 on FS19, but 1 seems OK
...
function xpCombine:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if self.isServer then
        ...
        if self:getIsTurnedOn() and self.movingDirection~=-1 and cutterIsTurnedOn then
            ...
            local multiplier =  spec.lastMultiplier --spec.mrCombineLimiter.basePerfAvgArea / spec.mrCombineLimiter.yield
            -- print("multiplier "..tostring(multiplier))
            spec.mrCombineLimiter.totalArea = spec.mrCombineLimiter.totalArea + (spec.lastArea * multiplier);
            ...
            if spec.mrCombineLimiter.currentTime>spec.mrCombineLimiter.avgTime or spec.mrCombineLimiter.totaldistance>spec.mrCombineLimiter.distanceForMeasuring then
                ...
                -- FS22: 1.375 wheat fert 0%   bonus 38% 12.3 18.5 T/ha
                -- FS25: 2.45  wheat fert 0%   bonus 22%  8.16 T/ha
                -- FS25: 4.00  wheat fert 100% bonus 97% 12.32 T/ha
        -- if Vehicle.debugRendering then
                    ...
                    -- print("spec.mrCombineLimiter.currentTime"..tostring(spec.mrCombineLimiter.currentTime))
                    local tonPH = 3600000 * spec.mrCombineLimiter.totalOutputMass / spec.mrCombineLimiter.currentTime
                    spec.mrCombineLimiter.tonPerHour = tonPH --equivalentSqmPerHour * fruitDesc.literPerSqm * massPerLiter
                    -- print("MathUtil.areaToHa(spec.mrCombineLimiter.totalArea, g_currentMission:getFruitPixelsToSqm())"..tostring(MathUtil.areaToHa(spec.mrCombineLimiter.totalArea, g_currentMission:getFruitPixelsToSqm())))
                    local yield = 0.
                    if spec.mrCombineLimiter.totalArea > 0. then
                        yield = spec.lastMultiplier * spec.mrCombineLimiter.totalOutputMass / MathUtil.areaToHa(spec.mrCombineLimiter.totalArea, g_currentMission:getFruitPixelsToSqm())
                    end
                    if (yield > 0.) == (spec.mrCombineLimiter.currentAvgArea > 0.) then
                        spec.mrCombineLimiter.yield = yield + (0.02 * yield * math.random(-1, 1))
                    end
                    -- print("equivalentSqmPerHour"..tostring(equivalentSqmPerHour))
                    -- local yieldTPH = 10000 * yield / equivalentSqmPerHour
                    tonPH = equivalentSqmPerHour * fruitDesc.literPerSqm * massPerLiter
                    -- print("tonPerHour "..tostring(spec.mrCombineLimiter.tonPerHour))
                    -- print("yieldTPH   "..tostring(yieldTPH))
                    -- print("tonPH      "..tostring(tonPH ))
                    -- spec.mrCombineLimiter.engineLoad = 0.1 * tonPH / yield --spec.mrCombineLimiter.currentAvgArea / spec.mrCombineLimiter.basePerfAvgArea
                    -- local minYield = 
                    -- spec.lastMultiplier = yield / minYield
        -- end

                end
                -- print("spec.mrCombineLimiter.basePerfAvgArea"..tostring(spec.mrCombineLimiter.basePerfAvgArea))
...
function xpCombine:addCutterArea(superfunc, area, liters, inputFruitType, outputFillType, strawRatio, strawGroundType, farmId, cutterLoad)
    local spec = self.spec_xpCombine

    local ret = superfunc(self, area, liters, inputFruitType, outputFillType, strawRatio, strawGroundType, farmId, cutterLoad)
    -- TODO: We don't have anymore realArea (with multiplier)
    spec.lastLiters = liters
    -- if xpCombine.debug then print("area: "..tostring(area).." - liters: "..tostring(liters)) end
    -- if xpCombine.debug then print("ret: "..tostring(ret)) end
